...
describe('pcs-stats', () => {
    it('should parse pcs stats create event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room created | token 578ae9c5-9875-471e-96f3-5f06c3e5534a _entity 56c2cd0a-7d9e-4b76-85be-944ae3b32ee9 sinceCreation 0ms sincePrevious -ms +1ms';
    ...
    it('should parse pcs stats PC connection stats change event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room PC connection state change to "connecting" | token ee65e403-0e76-41d7-8b4a-affd9ed9360b _entity 56c2cd0a-7d9e-4b76-85be-944ae3b32ee9 sinceCreation 732ms sincePrevious 1ms +1ms';
        const { body: log, head } = prepareLog(fullLog);
        expect(parser(log, head)).toStrictEqual({
            parser: 'room-pcs-stats',
    ...
    it('should parse pcs stats ICE candidate received event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room ICE candidate n°1 received "candidate:0 1 UDP 2122252543 192.168.68.108 59772 typ host" | token 7cc7f2d8-7a38-42a7-bac5-98433134b2ad _entity 56c2cd0a-7d9e-4b76-85be-944ae3b32ee9 sinceCreation 1028ms sincePrevious 130ms +127ms';
        const { body: log, head } = prepareLog(fullLog);
        expect(parser(log, head)).toStrictEqual({
            parser: 'room-pcs-stats',
            token: '7cc7f2d8-7a38-42a7-bac5-98433134b2ad',
    ...
    it('should parse pcs stats ICE candidate emitted event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room ICE candidate n°3 received "candidate:0 1 UDP 2122252543 192.168.68.108 52159 typ host" | token f3d0a037-f053-4c1c-8c58-7aa06d599b4e _entity 56c2cd0a-7d9e-4b76-85be-944ae3b32ee9 sinceCreation 2156ms sincePrevious 2156ms +12ms';
        const { body: log, head } = prepareLog(fullLog);
        expect(parser(log, head)).toStrictEqual({
            parser: 'room-pcs-stats',
    ...
    it('should parse pcs stats ready event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room ready | token 7cc7f2d8-7a38-42a7-bac5-98433134b2ad _entity 56c2cd0a-7d9e-4b76-85be-944ae3b32ee9 sinceCreation 1112ms sincePrevious 84ms +82ms';
        const { body: log, head } = prepareLog(fullLog);
        expect(parser(log, head)).toStrictEqual({
            parser: 'room-pcs-stats',
            token: '7cc7f2d8-7a38-42a7-bac5-98433134b2ad',
    ...
    it('should parse pcs stats deleted event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room deleted | token 833b09c3-1c27-4046-ac14-1f178ff6c9e7 _entity 56c2cd0a-7d9e-4b76-85be-944ae3b32ee9 sinceCreation 28020ms sincePrevious 28020ms';
        const { body: log, head } = prepareLog(fullLog);
        expect(parser(log, head)).toStrictEqual({
            parser: 'room-pcs-stats',
    ...
    it('should parse pcs stats created event correctly', () => {
        const fullLog =
            'api:debug:pcs-stats:room Created | token 02010338-170f-40ee-814d-fdc41db86d53 _entity e815eae8-98ad-4dd7-b470-acbccd4db0a5';
        const { body: log, head } = prepareLog(fullLog);
        expect(parser(log, head)).toStrictEqual({
            parser: 'room-pcs-stats',
